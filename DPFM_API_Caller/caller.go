...
func (c *DPFMAPICaller) deleteSqlProcess(
	...
	var countryData *dpfm_api_output_formatter.Country
	for _, a := range accepter {
		switch a {
		case "Country":
			h := c.countryDelete(input, output, log)
			countryData = h
			if h == nil {
				continue
			}
...
func (c *DPFMAPICaller) countryDelete(
	input *dpfm_api_input_reader.SDC,
	output *dpfm_api_output_formatter.SDC,
	log *logger.Logger,
) *dpfm_api_output_formatter.Country {
	sessionID := input.RuntimeSessionID
	country := c.CountryRead(input, log)
	country.IsMarkedForDeletion = input.Country.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": country, "function": "CountryCountry", "runtime_session_id": sessionID})
	if err != nil {
	...
	if !checkResult(res) {
		...
		output.SQLUpdateError = "Country Data cannot delete"
		return nil
	}
